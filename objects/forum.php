...
//	private $conn;
	...
	function readAll ($fid = 0, $page = null, $from_record_num = null, $records_per_page = null) {
		$lim = '';
...
//			print($row);
			$row['slink'] = $row['link'];
			$row['link'] = $this->link = $this->bLink.'/'.$row['link'];
...
*/			$this->forumsList[$row['fid']][] = $row;
	...
	function readOne() {
		...
		// update view
		$this->views++;
		$queryV = "UPDATE " . $this->table_name . " SET views  = :views WHERE id = :id";
		$stmt = $this->conn->prepare($queryV);
