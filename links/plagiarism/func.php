...
//				$str = implode('', array_slice($_ar, $i, $token));
				...
				$str = $strp.$strn;
				//echo $str.'~~~~~~~';
				$intT = get_hash_of_token($strp)+get_hash_of_token($strn);
...
function showDetection (array $compareAr, array $pair, bool $isEg, bool $usingML = false) {
if ($pair) {
	...
	foreach ($pair as $k => $p) {
		if ($k != 2) {
//			$content = $txtAr[$p];
			$content = file_get_contents('data/'.$p.'.format.cpp');
