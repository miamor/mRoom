...
function sce (a) {
	$(a).find('textarea').not('.one-cmt textarea, .non-sce, .non-me, .ace_text-input, .add-comment-line-textarea').each(function () {
		$(this).sceditor({
//			plugins: "bbcode",
...
function left (id) {
	...
	if (id == 'file') {
		if (!$('#left-menu .file').text().length) {
			$.get(MAIN_URL+'/files?temp=left', function (data) {
				if (!data) $('#left-menu ul.file').html('<div class="need-login">You need to <a href="'+MAIN_URL+'/login">log in</a> to use this feature.</div>');
				else $('#left-menu .file').html('<label>Files</label>'+data);
				$('#left-menu .file .one-menu').removeClass('active');
				var rdir = window.location.href.split('?dir=')[1].split('/')[0];
				$('#left-menu .file .one-menu a[data-dir="'+rdir+'"]').parent().addClass('active');
			});
		}
	} else if (id == 'blog') {
		if (!$('#left-menu .blog').text().length) {
			...
			$.get(href, function (data) {
				if (!data) $('#left-menu ul.blog').html('<div class="need-login">You need to <a href="'+MAIN_URL+'/login">log in</a> to use this feature.</div>');
					...
					$('#left-menu ul.blog .left-menu-one:eq(7)').before('<label>Cuộc thi</label>')
					$('#left-menu ul.blog .left-menu-one:eq(10)').before('<label>Write up</label>')
				}
			});
		}
	}
