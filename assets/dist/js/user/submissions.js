...
//	console.log(siO);
	...
	$.ajax({
		url: MAIN_URL+'/p?do=getPlaDetails',
		type: 'post',
		data: {
			cont1: siO.p1,
			cont2: siO.p2,
...
function showCompile (data, output, showAllTests) {
	...
	if (!data) $('#me-sub-compile').html('<div class="alerts alert-error">Oops! Missing data!</div>');
		...
		if (showAllTests == true) {
			...
			for (var i in tests) {
				test = tests[i];
				testCases += '<div class="tc-one tc-'+test['checkTxt']+'"><div class="tc-test"><a href="#t'+i+'" id="'+i+'" class="test-details">#'+i+'</a></div><div class="tc-input">'+test['input']+'</div><div class="tc-output">'+test['output']+'</div><div class="tc-time">'+test['time']+'</div><div class="tc-result submission-scored submission-'+test['checkTxt']+'"><img class="media-object" src="'+IMG+'/'+test['checkTxt']+'.png" width="12" height="12"> '+test['checkTxt']+'</div><div class="clearfix"></div></div>';
			}
			activeTab('noti');
...
function submissionList () {
	$('.p-submissions-list .ranker').click(function () {
		if (!$(this).hasClass('active')) {
			...
			checkPla('iid='+pid+'&fnum='+fid+'&mode='+fmode+'&uid='+$this.attr('data-uid'));
